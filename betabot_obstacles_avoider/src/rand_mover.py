...
def callback(data):
		...
		range_center.data= data.ranges[1499] 
		range_left.data= data.ranges[0]
		range_right.data= data.ranges[999]
		range_left_last.data= data.ranges[1749]
		range_right_last.data= data.ranges[1249]
...
def move():
    ...
    rospy.init_node('rand_mover', anonymous=True)
